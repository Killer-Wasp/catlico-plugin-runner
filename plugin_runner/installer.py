...
#: repo, or a credential prompt must fail rather than block forever (and pin an
#: ``asyncio.to_thread`` worker slot in ``install_from_source``).
...
def _default_git_run(argv: list[str]) -> subprocess.CompletedProcess:
    ...
    ``BatchMode`` command make an auth-required URL fail fast instead of
    blocking on a username/password/passphrase prompt, and a hard ``timeout``
    caps every call so a hanging remote or huge repo cannot block forever. A
    breached timeout surfaces as ``subprocess.TimeoutExpired``, which
    ``clone_source`` translates to ``GitCloneError``.
    """
    env = {
        **os.environ,
...
def _reject_dashed(label: str, value: str) -> None:
    """Reject a value git would parse as an option. Argv lists already stop
    *shell* injection, but a leading-dash ``source_ref``/``source_url`` still
    reaches git as a flag (e.g. ``--upload-pack=...`` on a clone URL is RCE on
    local transport — CVE-2018-17456 class; ``--orphan``/``-f`` on a checkout).
    The ``--`` end-of-options separator below is the primary defence; this is a
    belt-and-suspenders guard on values that are DB-stored and thus attacker-
    influenced."""
    if value.startswith("-"):
    ...
    *,
    ...
    Supported ``source_ref`` kinds:
      ...
      - raw commit SHA (full or abbreviated): ``--branch`` cannot target a
        ...
        a blobless full clone (``--filter=blob:none``) followed by
        ``git checkout <ref>``. ``--filter=blob:none`` caps the DoS surface of
        the fallback (only commit/tree objects are fetched up front, blobs are
        lazily fetched on checkout) without the correctness hazards of guessing
        a ``--depth`` for an arbitrary commit; it is still heavier than the
        shallow branch path.

    Security: every git invocation is an argv list passed to ``run`` (default:
    a non-interactive, timeout-bounded ``subprocess.run`` wrapper) — never
    ``shell=True`` and never string interpolation into a shell — so shell
    metacharacters in ``source_url``/``source_ref`` cannot execute anything.
    Beyond that, both values are guarded against leading-dash option injection
    (``_reject_dashed``) and every git command uses a ``--`` end-of-options
    separator so a crafted URL/ref cannot be parsed as a git flag. ``run`` is
    injectable so tests can point this at a local ``file://`` checkout with no
    network, or fake failures/timeouts without shelling out at all.

    Raises ``GitCloneError`` with the underlying git stderr/stdout on any
    failure (bad URL, unresolvable ref, corrupt repo, timeout, ...).
    """
    _reject_dashed("source_url", source_url)
    _reject_dashed("source_ref", source_ref)

    def _git(argv):
        # Single translation point for timeouts, covering both the real
        # timeout-bounded runner and any injected ``run`` fake that raises.
        try:
            return run(argv)
    ...
    if shallow.returncode != 0:
        ...
        # From here the repo is fully on disk; any failure must not leave it behind.
